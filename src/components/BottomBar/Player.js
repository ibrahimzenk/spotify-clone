...
function Player() {
  ...
  return (
    <div className="flex px-4 justify-between items-center h-full">
              ...
              {!sidebar && (
                <div className="w-14 h-14 mr-3 flex-shrink-0 group relative">
                  ...
                  <button
                    ...
                    className="w-6 h-6 flex items-center justify-center text-white bg-black rounded-full opacity-0 group-hover:opacity-60 hover:!opacity-100 hover:scale-[1.06] absolute top-1 right-1"
                  >
      ...
      <div className="flex flex-col items-center max-w-[45.125rem] w-[40%] px-3">
        <div className="flex items-center gap-x-2">
          ...
          <button onClick={controls[state?.playing ? "pause" : "play"]} className="w-8 h-8 flex items-center justify-center bg-white text-black rounded-full hover:scale-[1.06]">
      ...
      <div ref={fsRef}>{isFullScreen && <FullScreenPlayer toggle={toggle} controls={controls} state={state} volumeIcon={volumeIcon} />}</div>
    </div>
  );
}
